...
def _create_overview_sheet(ws, results: list[ProductAnalysis]) -> None:
    ...
    for row_idx, result in enumerate(results, 2):
        ...
        # Webshop readiness
        ws_status = result.webshop_status or ""
        ws_cell = ws.cell(row=row_idx, column=11, value=ws_status)
        if ws_status == "Klar":
            ws_cell.fill = PatternFill(start_color="C6EFCE", end_color="C6EFCE", fill_type="solid")
            ws_cell.font = Font(color="006100", bold=True)
        elif ws_status == "Delvis klar":
            ws_cell.fill = PatternFill(start_color="FFEB9C", end_color="FFEB9C", fill_type="solid")
            ws_cell.font = Font(color="9C6500")
        elif ws_status == "Ikke klar":
...
def _create_summary_sheet(ws, results: list[ProductAnalysis],
    ...
    ws_ready = sum(1 for r in results if r.webshop_status == "Klar")
    ws_partial = sum(1 for r in results if r.webshop_status == "Delvis klar")
    ws_not_ready = sum(1 for r in results if r.webshop_status == "Ikke klar")
    ws_unknown = total - ws_ready - ws_partial - ws_not_ready
    rows.append(("NETTBUTIKK-KLAR", "", 0))
    rows.append(("Klar", ws_ready, 1))
    rows.append(("Delvis klar", ws_partial, 1))
